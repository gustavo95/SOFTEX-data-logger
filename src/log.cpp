...
void Log::init()
{
  ...
  try
  {
    ...
    // rtc.adjust(DateTime(2021, 02, 26, 17, 25, 00));
    now = rtc.now();
...
int Log::dataloggerDataSend(long date, float data[]){
  DataEncDec encoder(89); // 1 + 4 + 1*20 + 2*20 + 3*8 = 89
  encoder.addHeader(ThisDevice, GATEWAY);
  encoder.addDate(date);
  ...
  for(int i = 0; i < 6; i++){
    for(int j = 0; j < 8; j++){
      if((i < 2) || ((i == 2) && (j<4))){
        encoder.addCurrent(data[k]);
      }else if(i < 5){
        encoder.addVoltage(data[k]);
      }else{
        encoder.addPower(data[k]);
      }
      k++;
    }
  }
